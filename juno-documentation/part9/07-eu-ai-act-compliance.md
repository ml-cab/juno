...
**Current state: IMPLEMENTED.** A dedicated `AiDisclosure` class in the coordinator module is the single source of truth for the disclosure field name, its default text, and the opt-out resolution logic.

- `OpenAiChatHandler.handleBlockingChat` includes `x_juno_ai_disclosure` (a short text notice) in the response root, alongside `x_juno_latency_ms`.
- `OpenAiChatHandler.handleStreamingChat` includes the same field in the first SSE event (`TokenConsumer.onPrefillComplete`), before any generated token, so the notice is never buried mid-stream.
- `OaiChatCompletionRequest` accepts a request-level opt-out, `x_juno_disclosure: false`, for API-to-API integrations where no natural person is present to receive the notice. Default is enabled; only an explicit `false` disables it.
- `InferenceApiServer.toResponse` includes the same field on the native `/v1/inference` response for parity.
- `ConsoleMain.banner()` prints the disclosure text once at startup for both `local` and `cluster` REPL modes.
- `InferenceApiServer.CONSOLE_HTML` shows the same disclosure text once, as a static footer line in the embedded web console.
- `juno-api.yaml` documents `x_juno_ai_disclosure` (response) and `x_juno_disclosure` (request, default true) on the relevant schemas.

**Residual scope:** the native `/v1/inference/stream` SSE endpoint (as opposed to the OpenAI-compatible streaming endpoint) does not currently carry the disclosure field, and has no opt-out field of its own. `ConsoleMain`'s LoRA training REPL (`runLoraRepl()`) does not print the banner line, because it is a fine-tuning workflow rather than a conversational deployment. Operators serving `/v1/inference/stream` directly to end users should account for this until a follow-up closes the gap.
...
|---|---|---|---|---|
| User transparency / AI disclosure | 50 | Limited+ | Implemented (`AiDisclosure`, API + console + REPL); native SSE stream still open | LOW (was CRITICAL) |
...
**1. Implement Article 50 AI disclosure (immediate, low effort): DONE**
Shipped as `AiDisclosure` (coordinator module), consumed by `OpenAiChatHandler` (blocking and streaming), `InferenceApiServer` (native `/v1/inference` and the embedded console), and `ConsoleMain` (local and cluster REPL banners). Opt-out available via `x_juno_disclosure: false` for API-to-API integrations. Remaining follow-up: extend to the native `/v1/inference/stream` SSE endpoint.
...
The engine is technically sophisticated and operationally well-designed. Its fault tolerance, observability, and structured API make it a credible foundation for compliant deployments. Article 50 AI disclosure is now implemented (`AiDisclosure`, wired into the OpenAI-compatible API, the native inference API, the embedded console, and the REPL banners). The codebase still provides no audit logging, no authentication, no human oversight hooks, and no data governance for the LoRA training pipeline.

An operator deploying Juno in a limited-risk context (general chat assistant) no longer faces the Article 50 gap out of the box. An operator deploying Juno in a high-risk context (employment screening, credit, healthcare triage, etc.) would still face the full Chapter III obligation set and would need to build substantial compliance infrastructure on top of the current engine.

The most impactful near-term investment for the Juno project is now: structured audit logging and API authentication, plus closing the residual Article 50 gap on the native `/v1/inference/stream` endpoint, three changes that collectively address the most urgent remaining regulatory exposure across all deployment contexts.

---

[<- 9.6 Export Control](#ch-9-6) &nbsp;|&nbsp; [Table of Contents](../index.md) &nbsp;|&nbsp; [9.8 Disclaimer of Warranties ->](#ch-9-8)