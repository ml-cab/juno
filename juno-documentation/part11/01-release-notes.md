...
- **Partial OpenAI compatibility**: `logit_bias` and `user` remain ignored for client compatibility. `stop`, `seed`, and `presence_penalty` are honored on `/v1/chat/completions`. `response_format` `json_object` / `json_schema` constrain decode; unsupported schema keywords return HTTP 400.
