...
public final class InferenceApiServer {
	 ...
	 * /health/probe and /health-data are proxied to the sidecar so the health
	 * dashboard can be served from the coordinator's own port.
	 */
	...
	/**
	 * Proxy {@code POST /health/probe} → sidecar {@code POST /health/probe}. Nodes
	 * deployed by juno-deploy.sh point {@code JUNO_HEALTH_URL} at the coordinator
	 * (port 8080) so all health data flows through one endpoint.
	 */
	private void handleHealthProbeProxy(Context ctx) {
		...
		try {
			...
			java.net.http.HttpRequest req = java.net.http.HttpRequest.newBuilder()
					...
					.POST(java.net.http.HttpRequest.BodyPublishers.ofString(ctx.body())).build();
	...
	/**
	 * Proxy {@code GET /health-data} → sidecar {@code GET /health}. The health
	 * dashboard JavaScript fetches from the same origin (/health-data) so CORS is
	 * never an issue regardless of which port the user connects to.
	 */
	private void handleHealthDataProxy(Context ctx) {
		...
		try {
			java.net.http.HttpClient http = java.net.http.HttpClient.newHttpClient();
			java.net.http.HttpRequest req = java.net.http.HttpRequest.newBuilder()
					.uri(java.net.URI.create(healthSidecarUrl + "/health")).GET().build();
			java.net.http.HttpResponse<String> resp = http.send(req,
					java.net.http.HttpResponse.BodyHandlers.ofString());
			ctx.status(resp.statusCode()).contentType("application/json").result(resp.body());
		} catch (Exception e) {
	 ...
	 *
	 * Served at GET / — no external dependencies. Polls /v1/cluster/health every 10
	 * s; fetches /v1/models on load. Streams tokens via POST /v1/inference/stream
	 * using the Fetch ReadableStream API to consume the SSE response body.
	 *
	 * SSE event shape (from SseTokenConsumer): data: {"requestId":"...","token":"
	 * hi","tokenId":1234,"isComplete":false} data:
	...
	// to the sidecar) — no CORS issues, no extra port needed in AWS security
	// groups.
