 ...
 *
 * Core invariant: when the caller supplies a sessionId, subsequent turns in
 * the same conversation must NOT re-run the forward pass for tokens that were
...
class GenerationLoopSessionTest {
    ...
    private InferenceRequest sessionRequest(String sessionId, String userText) {
        return InferenceRequest.ofSession(sessionId,
                "llama3-8b",
                List.of(ChatMessage.user(userText)),
    ...
    void second_turn_advances_startPos_beyond_zero() {
        ...
        assertThat(minStartPosTurn2)
                .as("turn 2 must skip cached prefix — minStartPos must be > 0")
                .isGreaterThan(0);
    }
    ...
    void session_reuse_reduces_forward_calls_vs_no_cache() {
        ...
        loopWith(new StubInferencePipeline())
                .generate(sessionRequest(sessionId, "hello my name is dima"), TokenConsumer.discard());
        ...
        new GenerationLoop(tokenizer, sampler, new SpyInferencePipeline(noCachePositions), kvCache)
                .generate(
                        InferenceRequest.of("llama3-8b", turn2Msgs,
        ...
        assertThat(cachedCalls)
                .as("session reuse must issue fewer forward() calls than full re-prefill "
                        + "(noCacheCalls=" + noCacheCalls + ", cachedCalls=" + cachedCalls + ")")
                .isLessThan(noCacheCalls);
    }
    ...
    void stateless_request_evicts_kv_on_completion() {
        ...
        // Prefix cache for these tokens should NOT be populated (stateless — no cachePrefix call).
    ...
    void session_prefix_is_cached_after_first_turn() {
        ...
        loopWith(new StubInferencePipeline())
                .generate(sessionRequest(sessionId, "the quick brown fox"), TokenConsumer.discard());

        // Turn 2 — if the prefix was cached, forward() must be called with startPos > 0.
        List<Integer> positions = new CopyOnWriteArrayList<>();
        List<ChatMessage> turn2Msgs = new ArrayList<>();
        ...
        assertThat(minStartPos)
                .as("session turn 1 must populate the prefix cache — turn 2 must start at pos > 0")
                .isGreaterThan(0);
    }
    ...
    void evict_session_clears_prefix_cache() {
        ...
        List<ChatMessage> turn2Msgs = List.of(
        ...
        assertThat(minCold)
                .as("evictSession must remove the prefix cache entry — next run must start at 0")
                .isEqualTo(0);
    }
    ...
    void different_sessions_are_isolated() {
        ...
        List<ChatMessage> bTurn2Msgs = List.of(
                ChatMessage.user("session b first message"),
                ChatMessage.assistant("acknowledged"),
                ChatMessage.user("session b second message"));
        ...
        assertThat(minB)
                .as("session B prefix must survive eviction of session A — turn 2 startPos must be > 0")
                .isGreaterThan(0);
    }
    ...
    void ofSession_rejects_blank_session_id() {
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->
                InferenceRequest.ofSession("  ", "llama3-8b",
    ...
    void kv_cache_key_is_session_id_when_session_present() {
        String sessionId = UUID.randomUUID().toString();
        InferenceRequest req = InferenceRequest.ofSession(sessionId, "llama3-8b",
                List.of(ChatMessage.user("hi")), SamplingParams.defaults(), RequestPriority.NORMAL);
