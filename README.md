...
- **LoRA fine-tuning without touching the base model.** Adapter tensors, checkpoint I/O, and Adam live in the **`lora`** Maven module (`cab.ml.juno.lora`). `LoraTrainableHandler` in **`node`** loads GGUF weights, applies adapters on Q/V projections, and runs training; forward matmuls use GPU (FP16 device weights + cuBLAS) when `CudaMatVec` is selected. Backward through frozen weights still dequantises one block per row on CPU. Adapters persist to a `.lora` file; the GGUF is never modified. For a standalone merged model, use `./juno merge`.
