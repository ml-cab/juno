...
```
[Client]  HTTP: OpenAI-compatible (/v1/chat/completions, /v1/models) + Juno (/v1/inference, …) on Javalin; gRPC to nodes only
    |
[Coordinator]
...
| Module | Contents |
|--------|----------|
| `api` | OpenAPI 3.0 spec (`api/src/main/resources/openapi.yaml`), JAX-RS interfaces, `inference.proto`. OpenAI-compatible HTTP is documented in **`juno-api.yaml`** at the repo root. |
| `registry` | `NodeDescriptor`, `ShardPlanner`, `ShardMap`, `ParallelismType`, `TensorShardAssignment`, `TensorShardPlanner` |
| `coordinator` | `GenerationLoop`, `RequestScheduler`, `FaultTolerantPipeline`, `ProcessPipelineClient`, `TensorParallelPipelineClient`, inference DTOs and streaming token consumers (no embedded HTTP) |
| `juno-api-server` | `InferenceApiServer` (Javalin: Juno + OpenAI-compatible routes), `OpenAiChatHandler`, `OpenAiAdapter`, `JunoApiServerMain` (remote coordinator wiring + HTTP). Spec: `juno-api.yaml` at repo root. |
...
| `juno-master` | Fat jar (`juno-master.jar`). Manifest `Main-Class`: `CoordinatorMain` — thin wrapper that delegates to `JunoApiServerMain` (`cab.ml.juno.apiserver`) so the shaded artifact keeps a stable entry point while HTTP lives in `juno-api-server`. Same env vars as before (`JUNO_NODE_ADDRESSES`, `JUNO_MODEL_PATH`, `JUNO_PTYPE`, `JUNO_HTTP_PORT`, `JUNO_DTYPE`, `JUNO_MAX_QUEUE`, `JUNO_BYTE_ORDER`, optional `JUNO_HEALTH*`). No forking, no `ClusterHarness`. Launched by `juno-coordinator.service` on the AWS coordinator host. Integration tests in package `cab.ml.juno.master`; `ModelLiveRunnerIT` gated behind `-Pintegration` Maven profile. |
...
- No Spring Boot. Javalin for REST lives in `juno-api-server` (`InferenceApiServer`); `coordinator` stays HTTP-free.
- **OpenAI-compatible HTTP.** `POST /v1/chat/completions`, `GET /v1/models`, and `GET /v1/models/{id}` follow the public contract in `juno-api.yaml` (blocking and SSE streaming). Legacy Juno routes (`/v1/inference`, `/v1/inference/stream`, cluster health, unload) remain on the same server.
...
- **AWS infrastructure scripted.** `scripts/aws/launcher.sh` is a credential wrapper. `juno-deploy.sh` is a unified cluster lifecycle script: hardware is auto-detected during bootstrap — GPU nodes set `JUNO_USE_GPU=true` (CUDA is pre-installed in the golden AMI by `make-ami.sh`, not at boot), CPU nodes skip GPU setup. Commands: `setup | start | stop | teardown | status | scan-regions`. Coordinator can be co-located on node 1 (default, free) or launched as a separate t3.medium. In `--coordinator separate` mode the coordinator instance is launched first so its private IP can be baked into every node's `JUNO_HEALTH_URL` at creation time; the separate coordinator also runs with `-DJUNO_HEALTH=true` so it accepts health probes from nodes. GPU quota is checked before any instances launch — insufficient vCPUs fail hard with a link to the Service Quotas console rather than silently reducing node count. All SSH remote commands use absolute binary paths to work around the minimal PATH in EC2 non-login shells. State persisted to `~/.juno-deploy-state`; Ctrl+C auto-stops instances before exit. After `setup` the script bootstraps all nodes, writes `cluster-nodes.env`, starts the coordinator, and enters the live cluster monitor. Web console served at `http://<coordinator>:8080` once cluster is healthy.
