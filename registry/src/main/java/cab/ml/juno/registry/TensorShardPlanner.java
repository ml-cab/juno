 ...
 *
 * All eligible nodes are assigned the full transformer layer range [0, totalLayers).
 * Each node receives a unique tensorRank in [0, worldSize) that determines which
 * horizontal slice of the attention heads and FFN width it owns.
 *
 * Constraint: numHeads must be even (divisible by 2).
 * Transformer attention heads always pair up for RoPE sin/cos rotation, so an
 * odd head count is architecturally invalid regardless of node count.  Uneven
 * distribution across nodes is fine — heads are assigned by ceiling-division so
 * ranks differ by at most 1 (e.g. 32 heads × 3 nodes → 10 / 11 / 11).
 *
 ...
 *
 * Example — 3 nodes, 32 heads, 22 layers:
 *   Rank 0: heads [0,  10), layers [0, 22)
 *   Rank 1: heads [10, 21), layers [0, 22)   (rounding if not divisible)
 *   Rank 2: heads [21, 32), layers [0, 22)
 *
...
public final class TensorShardPlanner {
     ...
     * @return immutable list of TensorShardAssignment, one per eligible node, ordered by rank
     ...
     */
    public List<TensorShardAssignment> plan(
            String               modelId,
            int                  totalLayers,
            int                  numHeads,
        ...
        List<NodeDescriptor> eligible = nodes.stream()
                .filter(NodeDescriptor::isAvailable)
                .sorted(Comparator.comparingDouble(n -> -scorer.score(n, 1.0)))
                .toList();
