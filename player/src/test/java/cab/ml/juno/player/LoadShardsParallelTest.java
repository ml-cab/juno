 ...
 * 1. All N nodes receive a LoadShard RPC.
 * 2. loadShards() is parallel — nodes are contacted concurrently, not serially.
 *    Measured by checking that a shard-load with a deliberate per-node delay
 *    completes in roughly (1 × delay) rather than (N × delay).
 */
...
class LoadShardsParallelTest {
    ...
    /**
     * Tiny gRPC server that records received LoadShard calls and optionally
     * sleeps to simulate I/O latency.
     */
                    ...
                    .addService(new NodeServiceGrpc.NodeServiceImplBase() {
                        ...
                        public void loadShard(LoadShardRequest request,
                            ...
                            responseObserver.onNext(LoadShardResponse.newBuilder()
                                    .setMessage("ok layers " + request.getStartLayer()
                                            + "-" + request.getEndLayer())
                                    .build());
    ...
    void all_nodes_receive_load_shard() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < n; i++) {
                assertThat(servers.get(i).loadCount.get())
                        .as("node %d should have received 1 LoadShard call", i)
                        .isEqualTo(1);
            }
     ...
     *
     * Sequential loading: 3 × 300ms = ~900ms.
     * Parallel loading:   1 × 300ms = ~300ms (+ overhead).
     *
     * We assert total elapsed < 600ms — well below the sequential lower bound.
     * This test FAILS before the parallel fix and PASSES after.
     */
    @Test
    ...
    void load_shards_is_parallel_not_serial() throws Exception {
        ...
        try {
            ...
            assertThat(elapsed)
                    .as("loadShards must be parallel: expected ~%dms, got %dms", delayMs, elapsed)
