 ...
 * ─────────────────────────────────────────────────
 * ProcessPipelineClient (pipeline parallel — vertical / depth scaling):
 *   Activation flows node-0 → node-1 → node-2 in serial order.
 *   Each node computes a contiguous block of transformer layers.
 *   Network pattern: N-1 sequential gRPC hops per decode step.
 *
 * TensorParallelPipelineClient (tensor parallel — horizontal / width scaling):
 *   The SAME input tokens are sent to ALL nodes simultaneously.
 *   Each node computes ALL layers but only its horizontal weight slice
 *   (attention heads [headStart, headEnd) and FFN width slice).
 *   Each node returns a partial logit vector (same shape as full logits).
 *   The coordinator sums all partial vectors (star AllReduce) to produce
 *   the full next-token distribution.
 *   Network pattern: one broadcast + N parallel gRPC calls per decode step.
 *
 * AllReduce strategy
 * ─────────────────
 * This implementation uses a star AllReduce: the coordinator collects partial
 * results from all N nodes and sums them. This trades coordinator-side CPU
 * work (O(N * vocabSize) additions) for simpler wiring and no inter-node
 * communication — well-suited for commodity LAN clusters without InfiniBand.
 *
 ...
 *
 * Node configuration (set during loadShards)
 * ─────────────────
 * Every tensor-parallel node is loaded with:
 *   startLayer = 0, endLayer = totalLayers
 *   hasEmbeddings = true, hasOutputProjection = true
 *   tensorRank = [0, worldSize),  tensorWorldSize = worldSize
 *
 * This means every node independently:
 *   - decodes raw token IDs from the activation bytes (hasEmbeddings=true)
 *   - returns partial FLOAT32 logits (hasOutputProjection=true)
 *
...
public final class TensorParallelPipelineClient implements InferencePipeline {
    ...
    public TensorParallelPipelineClient(List<NodeAddress> nodes, int vocabSize) {
        ...
        log.info("TensorParallelPipelineClient created — worldSize=" + nodes.size()
                + ", vocabSize=" + vocabSize);
    }
    ...
    /**
     * Broadcast the token sequence to all tensor-parallel nodes in parallel,
     * then reduce (element-wise sum) the partial logit vectors they return.
     *
     * Step 1: encode token IDs as raw int32 bytes (big-endian).
     * Step 2: dispatch ForwardRequest to every node concurrently via virtual threads.
     * Step 3: wait for all nodes to respond (CompletableFuture.allOf).
     * Step 4: element-wise sum of all partial float[] logit arrays (AllReduce).
     * Step 5: return the summed logit array to GenerationLoop for sampling.
     *
     * The winner token is determined by the sum: if node-0 puts 100.0 on token 42
     * and node-1 puts 100.0 on token 42, the sum is 200.0 — the argmax is preserved.
     * In real weight-sliced inference each node produces a scaled partial contribution
     * that sums to the correct full logit.
     */
    @Override
        ...
        for (NodeStub stub : stubs) {
            futures.add(CompletableFuture.supplyAsync(() -> {
                ForwardRequest req = ForwardRequest.newBuilder()
                        ...
                        .setActivation(ByteString.copyFrom(tokenBytes))
                        .setDtype(cab.ml.juno.api.grpc.ActivationDtype.FLOAT32)
                        .build();
                ...
                if (!response.getError().isEmpty()) {
                    throw new RuntimeException(
                            "Tensor-parallel forward pass failed on node "
                            + stub.address + ": " + response.getError());
                }

                return ActivationCodec.decode(
                        response.getActivation().toByteArray(),
     ...
     * unique tensorRank and the shared tensorWorldSize. The node uses its rank
     * to slice the attention head and FFN weight matrices at inference time.
     *
    ...
    public void loadShards(List<TensorShardConfig> shards) {
        if (shards.size() != stubs.size())
            throw new IllegalArgumentException(
                    ...
                    + ") must equal nodes.size() (" + stubs.size() + ")");
        ...
        for (int i = 0; i < stubs.size(); i++) {
            ...
            LoadShardRequest req = LoadShardRequest.newBuilder()
                    ...
                    .setTensorRank(shard.tensorRank())
                    .setTensorWorldSize(shard.tensorWorldSize())
                    .build();

            futures.add(CompletableFuture.runAsync(() -> {
                LoadShardResponse response = stubs.get(idx).blockingStub.loadShard(req);
                log.info("Tensor-parallel node " + idx + " (rank=" + shard.tensorRank()
                        + ") shard load: " + response.getMessage());
            }));
        }
     ...
     *
     * In tensor-parallel mode all nodes receive:
     *   startLayer = 0, endLayer = totalLayers
     *   hasEmbeddings = true, hasOutputProjection = true
     *   tensorRank   = [0, tensorWorldSize)
     *   tensorWorldSize = number of tensor-parallel nodes
     */
    public record TensorShardConfig(
            int     startLayer,
            int     endLayer,
            boolean hasEmbeddings,
            boolean hasOutputProjection,
            int     tensorRank,
            int     tensorWorldSize
    ) {}
    ...
    /** Encode a token-ID array as big-endian int32 bytes (same format as ProcessPipelineClient). */
    ...
    private static float[] allReduceSum(List<CompletableFuture<float[]>> futures, int vectorLen) {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Tensor-parallel forward pass failed: " + cause.getMessage(), cause);
        }
        ...
        for (CompletableFuture<float[]> future : futures) {
            ...
            if (partial.length != vectorLen) {
                throw new IllegalStateException(
                        "Partial logit vector length " + partial.length
                        + " != expected vocabSize " + vectorLen);
            }
