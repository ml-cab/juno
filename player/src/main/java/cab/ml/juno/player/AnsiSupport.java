 ...
 * Terminal Processing (VTP) via the Windows console API, and rewires System.out
 * / System.err to raw UTF-8 byte streams.
 *
 * <p>
 * On Linux / macOS this is a no-op.
 *
 * <p>
 * Call {@link #enable()} as the very first statement of
 * {@code ConsoleMain.main()}, before any output is written.
 *
 * <p>
 * Requires {@code --enable-native-access=ALL-UNNAMED} (already in scripts).
 *
 ...
 * {@code invokeExact} throws WrongMethodTypeException when a non-void return
 * value is discarded at the call site; that exception is silently swallowed and
 * VTP is never enabled.</li>
 * </ul>
 */
public final class AnsiSupport {
	...
	private static void tryEnableVtp(int stdHandleId) {
		...
		for (String dll : new String[] { "kernelbase", "kernel32" }) {
			if (tryEnableVtpWith(dll, stdHandleId))
				return;
		}
	}
	...
	private static boolean tryEnableVtpWith(String dll, int stdHandleId) {
		try {
			...
			MethodHandle getStdHandle = linker.downcallHandle(lib.find("GetStdHandle").orElseThrow(),
					FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.JAVA_INT));

			MethodHandle getConsoleMode = linker.downcallHandle(lib.find("GetConsoleMode").orElseThrow(),
					FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.ADDRESS));

			MethodHandle setConsoleMode = linker.downcallHandle(lib.find("SetConsoleMode").orElseThrow(),
			...
			try (Arena arena = Arena.ofConfined()) {
				...
				if (ok == 0)
					return false; // not a real console (pipe / file)

				int current = modePtr.get(ValueLayout.JAVA_INT, 0);
				int desired = current | ENABLE_PROCESSED_OUTPUT | ENABLE_VIRTUAL_TERMINAL_PROCESSING;
