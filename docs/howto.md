...
| `--prefill-batch N` | adaptive (GPU + `static` schedule, local single-shard); `32` otherwise | cluster, local, master | Max prompt tokens per prefill GPU window (`JUNO_PREFILL_BATCH`). On the local single-shard REPL with `--schedule static` and a GPU backend, the default sizes the chunk to cover the whole prompt in one window when there is enough free VRAM (queried live), falling back to `32` when VRAM is tight, on CPU-only runs, or under `--schedule continuous` (unchanged fixed chunking). Pass an explicit `N` to override on any surface; use `1` for per-token batched prefill. Cluster REPL and `juno lora` keep the fixed `32` default regardless of backend. |
...
| `--model-draft PATH` | — | local | Draft model GGUF for `--spec-type draft-simple` (`JUNO_MODEL_DRAFT`). Loaded as an independent, always-single-shard pipeline sharing the target's GPU backend; must report the same vocabulary size as the target model. |
