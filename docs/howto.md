...
| `--schedule static\|continuous` | `static` | cluster, local, lora | KV layout (`JUNO_SCHEDULE`). `static` = dense in-process KV (default, no gather). `continuous` = paged block pool + gather-to-workspace. Continuous **batching engine** is a follow-up; with `--parallel > 1`, paged KV still runs under static micro-batch (startup WARNING). On `juno lora` **training**, continuous/page-size are explicit no-ops (ephemeral float KV + WARNING). Cluster: paged KV per node; continuous scheduler remains local/single-shard follow-up. |
...
**Environment overrides:** `MODEL_PATH`, `JUNO_USE_GPU`, `JUNO_GPU_LAYERS`, `JUNO_MMQ`, `JUNO_CACHE_TYPE_K`, `JUNO_CACHE_TYPE_V`, `JUNO_PARALLEL`, `JUNO_BATCH_WINDOW_MS`, `PTYPE`, `DTYPE`, `BYTE_ORDER`,
