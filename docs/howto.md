...
Production GPU loads use `LlamaTransformerHandler.load(path, ctx, new CudaMatVec(gpuCtx))`:
weights are dequantized once at load time and uploaded as `DeviceFloatMatrix` objects —
forward passes then copy only `x` and `y` across the PCIe bus, not the weight matrix.
`GpuContext` is `AutoCloseable`; close it after the handler is no longer needed.
Use `CpuMatVec.INSTANCE` instead to force the CPU path (no GPU required).
