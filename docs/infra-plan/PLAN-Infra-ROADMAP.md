...
9. **Tier 20** — prefill GPU-residency fixes: pinned host-staging memory (fixes a confound that also
   ...
   Finding 6's confound. See [`PLAN-Infra-Tier20.md`](PLAN-Infra-Tier20.md). **Proposed, not started.**
...
```
P0:  13A ✓  →  5†  →  1†  →  8†  →  Vector SIMD†  →  13B†  →  17†  →  18†  →  19    GATE: 0.5× tg open (Phi-3.5 **0.326×**); mistral 0.15× **met** († feature complete; 13B tile-kernel speed exit met; 18 feature complete, reporting/docs only; 19 not started — targets per-launch host/FFI overhead, not GEMV tiling)
P1:              6  →  14  →  15  →  16
