...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **P0** — kernel path        | 1. Tier 13 Phase A ✓ → 2. **Tier 5 → Tier 1** → 3. **Tier 8** → 4. **Vector SIMD track** (P0 gate input) → 5. **Tier 13 Phase B** → 6. **Tier 17** (GPU batched-prefill GEMM) → 7. **Tier 18** (tuned-lane bake-off generalization + recommended-flags docs) | 13A, 5, 1, 8, SIMD, 13B, 17, 18 | Phi-3.5 Q4_K_M GPU tg ≥ **0.5×** llama (currently **0.33×** with `--mmq on`); mistral-7b on 8 GiB ≥ **0.15×** with Tier 5 `auto` (**0.43×** — **met**) |
