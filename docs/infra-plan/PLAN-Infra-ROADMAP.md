...
| 17   | `PLAN-Infra-Tier17.md` | GPU batched-prefill GEMM (CUDA)         | P0 step 6                | **In progress** — Gap 1 done: `CudaMatVec.sgemm(DeviceHalfMatrix, ...)` now uses `cublasGemmEx` (new `CudaFp16GemmOps`) for batches `> HALF_SGEMM_BATCH_MAX`; parity test `CudaSgemmBatchedPrefillParityTest` green (batches 1/8/9/16/32/128, non-tile-aligned shape); live JFR confirms `cuda-resident-fp16-gemm` backend fires on real TinyLlama and Phi-3.5-mini prefill (`--prefill-batch 32`); Qwen3 confirmed via shared `backend.sgemm(DeviceHalfMatrix, X)` call site (`Qwen3TransformerHandler.java:683-696`), not yet live-verified (no loadable non-MoE Qwen3 GGUF fixture in `models/` this session — the only Qwen3-family fixture present is `general.architecture=qwen35`, not in Juno's supported-arch dispatch list; separate Model E2E gap, not a Tier 17 regression). Found and fixed a pre-existing, previously-untested correctness bug while adding the parity test: `sgemmHalfBatched`'s `MemorySegment.copy` D2H read used an unscaled `b * rows` byte offset (missing `* Float.BYTES`), corrupting/crashing multi-request FP16 decode batches (`--parallel` batch 2-8) whenever the offset wasn't 4-byte aligned — no test had exercised `sgemm(DeviceHalfMatrix, float[][])` before. Gap 2 (Q4_K/Q5_K/Q6_K dequant-to-FP16 kernel + `sgemm(DeviceQ4KMatrix, ...)` override) **not started**; full GPU bake-off, `compare-lora.sh`, `compare-vision.sh`, and doc updates still open. |
