 ...
 *
 * <p>
 * Reads node addresses from the environment, wires the inference pipeline,
 * calls loadShards() on all nodes in parallel, then starts the Javalin REST
 * server and blocks until shutdown.
 *
 * <p>
 * No forking. No ClusterHarness. Nodes must already be running (NodeMain
...
public final class CoordinatorMain {
	...
	public static void main(String[] args) throws Exception {
		...
		if ("true".equalsIgnoreCase(env("JUNO_HEALTH", "false"))) {
			...
			HealthMain.startBackground(healthPort, new HealthThresholds(healthWarn, healthCrit, healthStaleMs));
		...
		log.info("Coordinator starting — " + nodeCount + " nodes, pType=" + ptypeStr + ", dtype=" + dtype + ", port="
				+ httpPort);
		...
		if (healthUrl != null) {
			...
			cab.ml.juno.health.HealthReporter coordReporter = new cab.ml.juno.health.HealthReporter("coordinator",
					"coordinator", healthUrl, cab.ml.juno.health.HealthReporter.DEFAULT_INTERVAL_MS);
			coordReporter.startBackground();
			apiServer.setLatencyReporter(coordReporter);
			Runtime.getRuntime().addShutdownHook(Thread.ofVirtual().unstarted(coordReporter::stop));
		}
	...
	private static cab.ml.juno.node.InferencePipeline buildPipelinePipeline(
			List<ProcessPipelineClient.NodeAddress> nodes, LlamaConfig config, ActivationDtype dtype) {

		List<ProcessPipelineClient.ShardConfig> shards = splitLayersEvenly(config.numLayers(), nodes.size());
	...
	private static cab.ml.juno.node.InferencePipeline buildTensorPipeline(List<ProcessPipelineClient.NodeAddress> nodes,
			LlamaConfig config, ActivationDtype dtype) {

		int worldSize = nodes.size();
		List<TensorParallelPipelineClient.NodeAddress> tensorAddrs = nodes.stream()
				.map(a -> new TensorParallelPipelineClient.NodeAddress(a.host(), a.port())).toList();

		TensorParallelPipelineClient client = new TensorParallelPipelineClient(tensorAddrs, config.vocabSize());

		List<TensorParallelPipelineClient.TensorShardConfig> shards = new ArrayList<>();
		for (int rank = 0; rank < worldSize; rank++) {
			shards.add(new TensorParallelPipelineClient.TensorShardConfig(0, config.numLayers(), true, true, rank,
					worldSize));
		}
	...
	static List<ProcessPipelineClient.ShardConfig> splitLayersEvenly(int totalLayers, int nodeCount) {
		...
		for (int i = 0; i < nodeCount; i++) {
			...
			log.info("  node-" + (i + 1) + ": layers [" + start + ", " + end + ")" + (hasEmbed ? " +embed" : "")
					+ (hasOutput ? " +output" : ""));
			start = end;
		}
		return shards;
	...
	private static ModelRegistry buildRegistry(LlamaConfig config, String modelPath) {
		...
		ModelDescriptor descriptor = new ModelDescriptor(modelId, config.architecture(), config.numLayers(),
	...
	private static String env(String key, String defaultValue) {
		// Check JVM system property first (-Dkey=value), then OS env var.
		// This lets both -DJUNO_HEALTH=true and JUNO_HEALTH=true in the shell work.
