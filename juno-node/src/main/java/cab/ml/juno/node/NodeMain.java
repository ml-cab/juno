 ...
 * -Dnode.id=<nodeId> required -Dnode.port=<port> required
 * -Dmodel.path=<modelPath> optional (if missing, runs with dummy handler)
 * -DJUNO_USE_GPU=<true|false> optional, defaults to true
 * -Djuno.cuda.device=<0..N-1> optional CUDA device index for
 * {@link cab.ml.juno.node.GpuContext#shared(int)}
 *
 * Health sidecar (optional — starts alongside the node, does NOT replace it):
 * -Djuno.health=true enable the health HTTP sidecar -Djuno.health.port=8081
 * port for the health sidecar (default: 8081) -Djuno.health.staleMs=15000
 * staleness threshold in ms (default: 15000) -Djuno.health.warn=0.90 VRAM
 * warning threshold (default: 0.90) -Djuno.health.critical=0.98 VRAM critical
 * threshold (default: 0.98)
 *
 * Example: java -Dnode.id=node-1 -Dnode.port=9092
 * -Dmodel.path=/models/model.gguf \ -Djuno.health=true -Djuno.health.port=8082
 * \ -jar juno-node.jar
 *
 * The old command-line signature (nodeId port modelPath) is still supported for
 * backward compatibility, but system properties take precedence.
 */
public final class NodeMain {
	...
	public static void main(String[] args) throws Exception {
		...
		if (healthUrl != null && !healthUrl.isBlank()) {
			...
			Runtime.getRuntime().addShutdownHook(Thread.ofVirtual().unstarted(reporterRef::stop));
