...
**Juno 0.1.1 — LoRA on GPU, finally fast**

Juno is a pure-Java distributed LLM inference/fine-tuning engine — GGUF loading, pipeline/tensor parallel across JVM nodes via gRPC, CUDA/ROCm acceleration through the Panama FFI (`java.lang.foreign`), LoRA training built in, and an OpenAI-compatible REST API via Javalin. Requires JDK 25+, Maven 3.9+.

**0.1.1** is the LoRA release: real GPU training, multi-arch adapters, a rebuilt docs site, AI disclosure on outputs, and Windows parity for API / play / merge.
...
| | **0.1.1 CUDA** | **0.1.1 CPU** | **0.1.0 GPU** | **0.1.0 CPU** |
|---|---:|---:|---:|---:|
...
JDK **25+**. Optional CUDA 12.x or ROCm 6+. CPU-only still works.

### Known limits

Text-only; OpenAI `n > 1` rejected; some OpenAI fields ignored; no built-in auth/TLS; exact K-quant QA-LoRA merge unsupported.

**License:** Apache 2.0 · **Docs:** https://ml.cab/juno-documentation · **GitHub:** https://github.com/ml-cab/juno
