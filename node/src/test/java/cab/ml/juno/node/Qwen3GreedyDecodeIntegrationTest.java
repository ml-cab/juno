...
class Qwen3GreedyDecodeIntegrationTest {
	...
	private static boolean modelPresent() {
		return Path.of("models/qwen3-4b-instruct-q4_k_m.gguf").toFile().exists()
				|| Path.of("../models/qwen3-4b-instruct-q4_k_m.gguf").toFile().exists();
	}

	private static Path modelPath() {
		Path p = Path.of("models/qwen3-4b-instruct-q4_k_m.gguf");
		return p.toFile().exists() ? p : Path.of("../models/qwen3-4b-instruct-q4_k_m.gguf");
	}
	...
	void greedyHelloResponse_producesFiniteTokens() throws Exception {
		...
		Qwen3TransformerHandler handler = Qwen3TransformerHandler.load(modelPath(), ctx, CpuMatVec.INSTANCE);
		String reqId = "qwen3-greedy";
		...
		for (int tok : generated) {
			assertThat(tok).isGreaterThanOrEqualTo(0).isLessThan(200000);
		}
	}
