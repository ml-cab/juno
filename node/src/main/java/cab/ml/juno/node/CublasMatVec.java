 ...
 *
 * Row-major to cuBLAS mapping:
 *   cuBLAS is column-major. A row-major matrix A[rows x cols] is identical in
 *   memory to the transpose of a column-major matrix A^T[cols x rows]. To
 *   compute y = A * x using cuBLAS we therefore call cublasSgemv with
 *   CUBLAS_OP_T (transpose), m=rows, n=cols, lda=cols. This asks cuBLAS to
 *   compute y = (A^T)^T * x = A * x. No data is copied or reordered.
 *
 * Memory management:
 *   Each sgemv call allocates device buffers, copies data, executes, copies
 *   result back, and frees. This is correct and simple (KISS). A future
 *   GpuMemoryPool optimisation can replace the per-call cudaMalloc/cudaFree
 *   with slab allocation — the interface contract is unchanged.
 *
 * Thread safety:
 *   cuBLAS handles are safe for concurrent calls from different Java threads.
 *   Each call has its own device buffers so there is no shared mutable state.
 */
public final class CublasMatVec implements GpuMatVec {
    ...
    public float[] sgemv(float[] A, float[] x, int rows, int cols) {
        ...
        try {
            // ── Copy host → device ────────────────────────────────────────────
            JCuda.cudaMemcpy(d_A, Pointer.to(A), bytesA,
                jcuda.runtime.cudaMemcpyKind.cudaMemcpyHostToDevice);
            JCuda.cudaMemcpy(d_x, Pointer.to(x), bytesX,
                jcuda.runtime.cudaMemcpyKind.cudaMemcpyHostToDevice);
            ...
            JCublas2.cublasSgemv(ctx.handle(),
                cublasOperation.CUBLAS_OP_T,
                cols,       // m — rows of the stored column-major matrix
                rows,       // n — cols of the stored column-major matrix
                ALPHA,
                d_A, cols,  // lda = cols (leading dimension of row-major A)
                d_x, 1,
                BETA,
                d_y, 1);

            // ── Copy device → host ────────────────────────────────────────────
            JCuda.cudaMemcpy(Pointer.to(y), d_y, bytesY,
