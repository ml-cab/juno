 ...
 *
 * Structurally mirrors CpuForwardPassHandler. Matrix-vector multiplies are
 * delegated to a GpuMatVec instance — in production a CublasMatVec backed by
 * cublasSgemv (org.bytedeco cuda), in tests a CpuMatVec or a custom stub.
 *
 * Two factory methods:
 *
...
public final class GpuForwardPassHandler implements ForwardPassHandler {
    ...
    // ── Factory — host weights (tests / CpuMatVec fallback) ──────────────────
    ...
    public static GpuForwardPassHandler load(Path modelPath, ShardContext context, GpuMatVec matVec)
        ...
        log.info("GpuForwardPassHandler loading: layers " + context.startLayer() + "–" + context.endLayer()
    ...
    // ── Factory — GPU-resident weights (production) ───────────────────────────
    ...
    // ── Constructor — GPU-resident weights ────────────────────────────────────
    ...
    private GpuForwardPassHandler(
        ...
        try {
            ...
            for (DeviceFloatMatrix d : uploaded) {
                try { d.close(); } catch (Exception ignored) { /* best effort */ }
            }
