 ...
 *
 * <p>Implements both inference and a complete training step. Frozen base
 * weights are kept quantised (Q4_K, Q8_0, etc.) for inference. During
 * a training step the same quantised weights are also used for the
 * <em>backward</em> transpose matmul — one row at a time, dequantised
 * on demand, so peak extra memory is O(hiddenDim) not O(model).
 *
 * <h2>LoRA configuration</h2>
 * Create a {@link LoraAdapterSet} and pass it to {@link #load}. Adapters are
 * typically applied to {@code wq} and {@code wv}. Any projection with no
 * registered adapter is left frozen and does not generate parameter gradients.
 *
 * <h2>Training loop</h2>
 * <pre>
 ...
 * Gradients do NOT propagate through the KV-cache entries from earlier
 * sequence positions. This is the standard simplification for LoRA training:
 * each position's backward is independent, so the total gradient is the sum
 * over positions. In practice this has negligible impact on quality for typical
 * fine-tuning sequences (≤ 512 tokens).
 *
 * <h2>Thread safety</h2>
 * {@link #forward} is safe to call concurrently for distinct request IDs.
 * {@link #trainStep} is NOT thread-safe — it mutates gradient accumulators.
 */
public final class LoraTrainableHandler implements ForwardPassHandler {
     ...
     * @param adapters   LoRA adapters (typically created with {@link LoraAdapterSet#qv})
     */
    public static LoraTrainableHandler load(Path modelPath,
                                             ShardContext context,
                                             LoraAdapterSet adapters) throws IOException {
        log.info("Loading LoRA handler: layers " + context.startLayer() + "–" + context.endLayer()
    ...
    private LoraTrainableHandler(GgufReader r, LlamaConfig cfg,
                                  ShardContext ctx,
                                  LoraAdapterSet adapters) throws IOException {
        this.cfg           = cfg;
     ...
     *
     * <p>For tokens [t₀, t₁, …, t_{n}], position {@code pos} predicts
    ...
    public float trainStep(int[] tokens, LoraAdamOptimizer optimizer) {
        ...
        for (int pos = 0; pos < T; pos++) {
            ...
            if (hasOutputProj) {
                ...
                float[] logits     = LlamaTransformerHandler.matVec(outputProj,
                                         allXNormFinal[pos], cfg.vocabSize(), H);
        ...
        for (int pos = 0; pos < T; pos++) {
            ...
            if (hasOutputProj) {
                ...
                float[] gradXNormFinal = transposedMatVec(outputProj, gradLogits,
                                                          cfg.vocabSize(), H);
            ...
            for (int li = L - 1; li >= 0; li--) {
                gradX = backwardLayer(gradX, li, pos, allStates[pos][li],
                                      kCache[li], vCache[li], pos + 1);
            }
        }
    ...
    private void applyLoraInPlace(float[] out, int li, String proj, float[] input) {
        ...
        for (int i = 0; i < out.length; i++) out[i] += delta[i];
    }
    ...
    /**
     * Forward pass through one layer, storing all intermediate activations
     * needed for the backward pass. Returns a {@link LayerState}; the caller
     * must also call {@link #computeLayerOutput} to get the updated {@code x}.
     */
    private LayerState forwardLayerStore(float[] x, int li, int pos,
                                          float[] kCacheLayer, float[] vCacheLayer) {
        int H     = cfg.hiddenDim();
        int kvDim = cfg.kvDim();
        int NH    = cfg.numHeads();
        ...
        for (int h = 0; h < NH; h++) {
            ...
            for (int t = 0; t < seqLen; t++) {
                ...
                for (int d = 0; d < Hd; d++) dot += q[qBase + d] * kCacheLayer[kOff + d];
                scores[t] = dot * scale;
            }
            ...
            float sum = 0f;
            for (int t = 0; t < seqLen; t++) { scores[t] = (float) Math.exp(scores[t] - max); sum += scores[t]; }
            for (int t = 0; t < seqLen; t++) { scores[t] /= sum; attnW[h][t] = scores[t]; }
            ...
            for (int t = 0; t < seqLen; t++) {
                ...
                float w = scores[t];
                for (int d = 0; d < Hd; d++) attnOut[outBase + d] += w * vCacheLayer[vOff + d];
            }
        }
    ...
    /**
     * Backpropagate through one transformer layer.
     * Accumulates gradients into LoRA adapters and returns dL/dx for the previous layer.
     * Uses truncated BPTT: no gradients flow backward through the KV cache
     * entries from earlier positions.
     */
    private float[] backwardLayer(float[] gradOut, int li, int pos,
                                   LayerState st, float[] kCacheLayer, float[] vCacheLayer,
                                   int seqLen) {
        int H    = cfg.hiddenDim();
        int I    = cfg.intermediateSize();
        int kvDim = cfg.kvDim();
        ...
        float[] gradXNorm2 = add(transposedMatVec(wGate[li], gradGate, I, H),
                                  transposedMatVec(wUp[li],   gradUp,   I, H));
        ...
        for (int h = 0; h < NH; h++) {
            ...
            for (int t = 0; t < seqLen; t++) {
                int vOff = t * kvDim + kBase;
                float d2 = 0f;
                for (int d = 0; d < Hd; d++) d2 += gradAttnOut_h[d] * vCacheLayer[vOff + d];
                dotWithV[t] = d2;
            }
            // Σ_t aw[t] * dotWithV[t]
            float sumDot = 0f;
            for (int t = 0; t < seqLen; t++) sumDot += aw[t] * dotWithV[t];
    ...
    private float[] gqa(float[] q, float[] kCache, float[] vCache, int seqLen) {
        ...
        for (int h = 0; h < H; h++) {
            ...
            for (int t = 0; t < seqLen; t++) {
                ...
                for (int d = 0; d < Hd; d++) dot += q[qBase + d] * kCache[kOff + d];
                scores[t] = dot * scale;
            }
            ...
            for (int t = 0; t < seqLen; t++) {
                ...
                float w = scores[t];
                for (int d = 0; d < Hd; d++) out[outBase + d] += w * vCache[vOff + d];
            }
        }
        return out;
    }
    ...
    static float[] rmsNormBackward(float[] x, float[] w, float[] gradOut) {
        ...
        float dot = 0f;
        for (int i = 0; i < n; i++) dot += gradOut[i] * w[i] * x[i];
     ...
     * Mathematically: R^T is the rotation by -angle, same formula as forward
     * but with negated sin.
     */
     ...
     * Dequantises one row at a time to avoid materialising the full float matrix.
     */
     ...
     * Block layout: d:f16, dmin:f16, sc[12], qh[32], qs[128] = 176 bytes per 256 elements.
     */
    ...
    /**
     * Reduce thread-local accumulator arrays into a single result.
     * locals[0] is reused as the output to avoid one extra allocation.
     */
    private static float[] scatterReduce(float[][] locals, int cols) {
        float[] y = locals[0];
        for (int t = 1; t < locals.length; t++) {
            float[] loc = locals[t];
            for (int c = 0; c < cols; c++) y[c] += loc[c];
        }
        return y;
    }
    /**
     * Fallback for quantisation types not yet covered by a dedicated case
     * (e.g. F16 type=1, BF16 type=30, Q4_0 type=2).
     *
     * <p>Parallelises over output columns: each thread computes one column of
     * A (via a standard-basis forward matVec), then dots it with v. Correct
     * for all quantisation types because it reuses {@link LlamaTransformerHandler#matVec}.
     * Slower than the dedicated cases because each of the {@code cols} matVec calls
     * processes all {@code rows} elements.
     *
     * <p>A WARNING is logged so you know to add a dedicated {@code transposedXxx}
    ...
    private static float[] transposedFallback(GgufReader.QuantizedTensor A, float[] v, int rows, int cols) {
        java.util.logging.Logger.getLogger(LoraTrainableHandler.class.getName())
            .warning("transposedFallback: no dedicated transpose for GGML type=" + A.type()
                + " (" + rows + "x" + cols + "). Training will be slow — add a transposedTypeXxx case.");
        ...
        IntStream.range(0, cols).parallel().forEach(c -> {
            ...
            float acc = 0f;
            for (int r = 0; r < rows; r++) acc += column[r] * v[r];
            y[c] = acc;
        });
        return y;
    }
