 ...
 *
 * <h3>Backward</h3> Given the upstream gradient dL/dDelta (shape: [outDim]):
 * 
 * <pre>
 ...
 * each training step.
 *
 * <h3>Thread safety</h3> NOT thread-safe. Each training request must use its
 * own adapter (or the caller must synchronise). Inference
 * ({@link #forward(float[])}) is read-only on A and B and is safe to call
 * concurrently.
 */
public final class LoraAdapter {
	...
	/**
	 * alpha / rank — pre-computed scaling factor applied in both forward and
	 * backward.
	 */
	...
	/**
	 * Restore an adapter from saved parameters (checkpoint loading). gradA and
	 * gradB start at zero.
	 */
	 ...
	 *
	 * <p>
	 * This is the additive perturbation to overlay on top of the frozen weight's
	 * output. The caller computes {@code frozenOut + lora.forward(x)}.
	 *
	 ...
	 *
	 * <p>
	 * Chain rule through {@code delta = scale × B × (A × x)}:
	 * <ul>
	 * <li>h = A × x
