 ...
 *
 * <p>Each adapter's A and B matrices get their own independent first-moment (m)
 ...
 * </pre>
 *
 * <p>Note: weight decay is intentionally NOT applied to B because B starts at
 * zero and weight decay would keep pulling it back toward zero, making the
 * adapter learn very slowly. Weight decay on A provides a light regulariser
...
public final class LoraAdamOptimizer {
    ...
    /**
     * Per-adapter moment buffers. Value is float[4][]:
     *   [0] = mA, [1] = vA, [2] = mB, [3] = vB
     */
     ...
     * @param weightDecay L2 regularisation on A matrix (0.01 is light; 0 to disable)
     */
     ...
     *
     * <p>Reads gradients from each adapter's {@code gradA}/{@code gradB}
     * accumulators and updates the corresponding {@code a}/{@code b} arrays
     * in-place. The caller is responsible for zeroing gradients before the
     * next backward pass.
     *
    ...
    public void step(LoraAdapterSet adapters) {
        ...
        for (LoraAdapter adapter : adapters.all()) {
            float[][] buf = state.computeIfAbsent(adapter, a -> new float[][] {
                new float[a.a.length], // mA
    ...
    private void updateParams(float[] param, float[] grad,
                               float[] m, float[] v,
        ...
        for (int i = 0; i < param.length; i++) {
            double g = grad[i];
            if (applyWeightDecay) g += weightDecay * param[i];   // L2 reg
