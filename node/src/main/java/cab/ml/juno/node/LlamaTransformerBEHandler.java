 ...
 */
public final class LlamaTransformerBEHandler implements ForwardPassHandler {

	private static final Logger log = Logger.getLogger(LlamaTransformerBEHandler.class.getName());
	 ...
	 *                  for
	 */
	public static LlamaTransformerBEHandler load(Path modelPath, ShardContext context) throws IOException {
		...
		try (GgufReader r = GgufReader.open(modelPath)) {
			...
			log.info("Model: " + cfg);
			return new LlamaTransformerBEHandler(r, cfg, context, CpuMatVec.INSTANCE);
		}
	}
	 ...
	 */
	public static LlamaTransformerBEHandler load(Path modelPath, ShardContext context, MatVec backend)
			throws IOException {
		log.info("Loading GGUF shard: layers " + context.startLayer() + "–" + context.endLayer() + "  embd="
		...
		try (GgufReader r = GgufReader.open(modelPath)) {
			LlamaConfig cfg = LlamaConfig.from(r);
			log.info("Model: " + cfg);
			return new LlamaTransformerBEHandler(r, cfg, context, backend);
		}
	}
	...
	private LlamaTransformerBEHandler(
	...
	private LlamaTransformerBEHandler(GgufReader r, LlamaConfig cfg, ShardContext ctx, MatVec backend)
			throws IOException {
		this.cfg = cfg;
		this.backend = backend;
	...
	/**
	 * Build a minimal {@link LlamaTransformerBEHandler} with random F32 weights for
	 ...
	 *                     dev/stub mode
	 */
	static LlamaTransformerBEHandler newTestInstance(
		...
		LlamaTransformerBEHandler h = new LlamaTransformerBEHandler(
